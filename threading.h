    ...
    for (int x = (xyChunk.x) * CHUNK_SIZE - 1; x < (xyChunk.x + 1) * CHUNK_SIZE + 1; x++) {
        for (int z = (xyChunk.y) * CHUNK_SIZE - 1; z < (xyChunk.y + 1) * CHUNK_SIZE + 1; z++) {
            ...
            for (int y = 1; y < scaledHeight; y++) {
                ...
                //repChunk->blockData.insert(make_pair(ivec3(x, y, z), BlockData(ivec3(x, y, z), blockType)));
                
                //"Decorations"

                if (cloudDensity < -0.85f) {
                    //repChunk->blockData.insert(make_pair(ivec3(x, CHUNK_SIZE * CHUNK_SIZE + 3 * CHUNK_SIZE - 1, z), BlockData(ivec3(x, CHUNK_SIZE * CHUNK_SIZE + 3 * CHUNK_SIZE - 1, z), CLOUD)));
                ...
                if (blockType == GRASS_BLOCK) {
                    ...
                    if (x >= (xyChunk.x + 1) * CHUNK_SIZE - radius|| x <= (xyChunk.x) * CHUNK_SIZE + radius ||
                        z >= (xyChunk.y + 1) * CHUNK_SIZE - radius|| z <= (xyChunk.y) * CHUNK_SIZE + radius) { goto notree; }
                    if (treeDistrib > 0.0 && treeDistrib <= 750) {
                ...
                if (blockType == GRASS_BLOCK && blockType != OAK_WOOD) {
                    ivec3 pos(x, scaledHeight + 1, z);
                    if (isAir(repChunk->block_data[repChunk->at(pos)].blockType))
                    //if (repChunk->blockData.find(pos) == repChunk->blockData.end() ||
                        //isAir(repChunk->blockData[pos].blockType))
                    {
                        if (randomNumberForGrass < 2000.0) {
                            //repChunk->blockData[pos] = BlockData(pos, GRASS);
                            repChunk->block_data[repChunk->at(pos)] = BlockData(pos, GRASS);
                        }
                        else if (randomNumberForPoppy < 500.0) {
                            //repChunk->blockData[pos] = BlockData(pos, POPPY);
                            repChunk->block_data[repChunk->at(pos)] = BlockData(pos, POPPY);
                        }
                        else if (randomNumberForOrchid < 200.0) {
                            //repChunk->blockData[pos] = BlockData(pos, BLUE_ORCHID);
...
bool shouldEmitFace(vec2 xyChunk, Chunk* cd, int x, int y, int z, int dx, int dy, int dz) {
    ...
    if (((nx <= xyChunk.x * CHUNK_SIZE - 1 || nz <= xyChunk.y * CHUNK_SIZE - 1)              ||
          nx >= (xyChunk.x + 1) * CHUNK_SIZE + 1 || nz >= (xyChunk.y + 1) * CHUNK_SIZE + 1) /* && cd->blockData[checkPos].blockType != OAK_LEAVES*/&& cd->block_data[cd->at(checkPos)].blockType != OAK_LEAVES) return false;
    ...
    return       isAir(cd->block_data[cd->at(checkPos)].blockType)        ||
                 //isAir(cd->blockData[checkPos].blockType)             ||
                 //cd->blockData[checkPos].blockType.isFlat             ||
                 cd->block_data[cd->at(checkPos)].blockType.isFlat        ||
                 //cd->blockData[checkPos].blockType == OAK_LEAVES      ||
                 cd->block_data[cd->at(checkPos)].blockType == OAK_LEAVES ||
...
void emitFace(Mesh& m, int baseX, int baseY, int baseZ,
    int face, Item blockType, float x, float y, float z, vec3 direction, vec3 position) {
    vec3 cMask[4];
    ...
    switch (face) {
        case 0: cMask[0] = { tintr, tintg, tintb }; cMask[1] = { tintr, tintg, tintb }; cMask[2] = { tintr, tintg, tintb }; cMask[3] = { tintr, tintg, tintb }; break; // -X
        case 1: cMask[0] = { tintr, tintg, tintb }; cMask[1] = { tintr, tintg, tintb }; cMask[2] = { tintr, tintg, tintb }; cMask[3] = { tintr, tintg, tintb }; break; // +X
        case 2: cMask[0] = { tintr, tintg, tintb }; cMask[1] = { tintr, tintg, tintb }; cMask[2] = { tintr, tintg, tintb }; cMask[3] = { tintr, tintg, tintb }; break; // -Z
        case 3: cMask[0] = { tintr, tintg, tintb }; cMask[1] = { tintr, tintg, tintb }; cMask[2] = { tintr, tintg, tintb }; cMask[3] = { tintr, tintg, tintb }; break; // +Z
        case 4: cMask[0] = { tintr, tintg, tintb }; cMask[1] = { tintr, tintg, tintb }; cMask[2] = { tintr, tintg, tintb }; cMask[3] = { tintr, tintg, tintb }; break; // -Y
        case 5: cMask[0] = { tintr, tintg, tintb }; cMask[1] = { tintr, tintg, tintb }; cMask[2] = { tintr, tintg, tintb }; cMask[3] = { tintr, tintg, tintb }; break; // +Y
    }
    ...
    if (blockType == TORCH) {
        ...
        case 2: v[0] = { x + 1,y + 0,z + 1 }; v[1] = { x + 1,y + 1,z + 1 }; v[2] = { x + 0,y + 1,z + 0 }; v[3] = { x + 0,y + 0,z + 0 };  break; // -X disgonal
        case 3: v[0] = { x + 1,y + 0,z + 0 }; v[1] = { x + 1,y + 1,z + 0 }; v[2] = { x + 0,y + 1,z + 1 }; v[3] = { x + 0,y + 0,z + 1 };  break; // +X diagonal
        }
    }
    ...
    uint32_t base = (uint32_t)(m.verts.size() / 12);
    for (int i = 0; i < 4; i++) {
        ...
        m.verts.push_back(cMask[i].x);
        m.verts.push_back(cMask[i].y);
        m.verts.push_back(cMask[i].z);
    }
    // Two triangles (0,1,2) (2,3,0)
    m.inds.push_back(base + 1); m.inds.push_back(base + 2); m.inds.push_back(base + 3);
    m.inds.push_back(base + 3); m.inds.push_back(base + 0); m.inds.push_back(base + 1);
    //int(0.0f)) % 4
}
...
void meshChunk(vec2 xyChunk, Chunk* cd, Mesh& out, vec3 direction, vec3 blockPosition) {
    ...
    for (int x = (xyChunk.x) * CHUNK_SIZE - 1; x < (xyChunk.x + 1) * CHUNK_SIZE + 1; ++x)
        for (int y = 0; y <= (CHUNK_SIZE * CHUNK_SIZE) + 2 * CHUNK_SIZE + 5; ++y) // + 2 * CHUNK_SIZE
            for (int z = (xyChunk.y) * CHUNK_SIZE - 1; z < (xyChunk.y + 1) * CHUNK_SIZE + 1; ++z) {
                /*Item blockType = cd->blockData[ivec3(x, y, z)].blockType; */ Item blockType = cd->block_data[cd->at(ivec3(x, y, z))].blockType;
                /*vec3 blockOrientation = cd->blockData[ivec3(x, y, z)].orientation;*/ vec3 blockOrientation = cd->block_data[cd->at(ivec3(x, y, z))].orientation;
                if (direction != vec3(0, 1, 0) && blockPosition == vec3(x, y, z)) {
                    /*cd->blockData[blockPosition].orientation = direction;         */  cd->block_data[cd->at(blockPosition)].orientation = direction;
                    /*blockOrientation = cd->blockData[ivec3(x, y, z)].orientation; */  blockOrientation = cd->block_data[cd->at(ivec3(x, y, z))].orientation;              
                }
                ...
                {
                    if (shouldEmitFace(xyChunk, cd, x, y, z, -1, 0, 0)) emitFace(out, x, y, z, 0, blockType, fx, fy, fz, blockOrientation, blockPosition);
                    if (shouldEmitFace(xyChunk, cd, x, y, z, +1, 0, 0)) emitFace(out, x, y, z, 1, blockType, fx, fy, fz, blockOrientation, blockPosition);
                    if (shouldEmitFace(xyChunk, cd, x, y, z, 0, 0, -1)) emitFace(out, x, y, z, 2, blockType, fx, fy, fz, blockOrientation, blockPosition);
                    if (shouldEmitFace(xyChunk, cd, x, y, z, 0, 0, +1)) emitFace(out, x, y, z, 3, blockType, fx, fy, fz, blockOrientation, blockPosition);
                    if (shouldEmitFace(xyChunk, cd, x, y, z, 0, -1, 0)) emitFace(out, x, y, z, 4, blockType, fx, fy, fz, blockOrientation, blockPosition);
                    if (shouldEmitFace(xyChunk, cd, x, y, z, 0, +1, 0)) emitFace(out, x, y, z, 5, blockType, fx, fy, fz, blockOrientation, blockPosition);
                }
                else {                    
                    emitFace(out, x, y, z, 0, blockType, fx, fy, fz, direction, blockPosition);
                    emitFace(out, x, y, z, 1, blockType, fx, fy, fz, direction, blockPosition);           
                    emitFace(out, x, y, z, 2, blockType, fx, fy, fz, direction, blockPosition);
                    emitFace(out, x, y, z, 3, blockType, fx, fy, fz, direction, blockPosition);
                }
            }
}
...
void generateChunkAt(vec2 xyChunk, Chunk* repChunk) {
    ...
    repChunk->indexOffset += (uint32_t)(m.verts.size() / 12);
